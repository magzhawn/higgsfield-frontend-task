...
- **2026-05-23 — Dropped `loading="lazy"` from image cells.** Alternative: keep it as defense-in-depth. Why: virtualization is the load-bearing gate for which items exist in the DOM — every mounted `<img>` is, by construction, near or inside the viewport. The browser's lazy-loading on top adds a ~1-frame intersection-check delay before paint on remount with zero benefit. Verified empirically: rapid 1↔8 column toggles produce zero duplicate network requests (Picsum's `max-age=2592000` `immutable` cache headers respected); the perceived "flash" is decoded-pixel release on unmount + re-decode on remount, which `loading="lazy"` wouldn't have prevented anyway.
