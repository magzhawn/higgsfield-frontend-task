...
**4. Media loading.** Native `loading="lazy"` is gone — the virtualizer already gates which items mount, and double-gating only adds a one-frame intersection delay before cached images paint. Videos use a Pexels-served thumbnail poster (so the paused cell looks like what the video will play), `preload="none"`, and a **3-concurrent playback cap** to bound CPU + decode pressure on mid-tier laptops. The playback manager picks the videos nearest viewport center and pauses everything else. R4's "loading priority reflects visibility" is enforced via `fetchpriority` annotation. A diagnostic at cols=2 found that visible cells were receiving only 11% of first-fetch slots across jump-scroll tests — the browser fires requests in DOM-insertion order, so visible cells competed equally with ~10 overscan neighbors for connection slots. The virtualizer-reported visible-row range is computed in `MediaFeed` and threaded to `MediaItem`; cells inside the visible band get `fetchpriority="high"`, overscan cells get `"low"`. Verified via Chrome's CDP: all 50 overscan cells across a 3-jump-scroll diagnostic received `Low` priority; all visible image cells received `High`. The downstream effect on network completion order depends on infrastructure: on HTTP/2 origins (Picsum, Pexels), stream multiplexing eliminates the per-origin connection-pool bottleneck that priority hints would otherwise gate. The annotation correctly expresses the visibility model to the browser and CDN; whether the CDN strictly enforces the hint is a property of the infrastructure.
...
- **Close the grow-direction flash.** When a cell legitimately needs more pixels (grow direction), the `<img>` clears its rasterization while the new source decodes — a brief flash bounded by network RTT. Partially masked by progressive JPEGs from both providers (`brief black → blurry → sharp`, verified via SOF2 marker). Two fixes attempted and empirically ruled out: CSS `background-image` on a `<div>` (Chrome doesn't atomic-swap when URL and dimensions change in the same frame) and layered `<img>` with the old image held at old dimensions (visually produces a small image in a black container at extreme grows). The architectural path is canvas-based rendering with manual rasterization control. Full investigation in `CLAUDE.md`.
