...
- **Close the grow-direction flash.** Documented in detail in `CLAUDE.md`'s decisions log. The flash hits when a cell legitimately needs more pixels (cell got bigger → new larger source needed → `<img>` clears its rasterization while the new source decodes). Partially masked already by the fact that both providers serve progressive JPEGs: the cell goes black for ~150–200ms (network RTT before any bytes arrive), then shows a blurry preview that refines for the rest of the load window — felt experience is "brief black → blurry → sharp," not "solid black until decoded." Two fixes attempted and rejected on empirical evidence: (1) CSS `background-image` on a `<div>` — Chrome does NOT atomic-swap the previously-decoded background when the URL changes and the element's dimensions change in the same frame; (2) layered `<img>` with the old at old pixel dims and the new at new pixel dims — mechanism works but the visual is a small image floating in a large black container during the load window (~6% image / 94% black at extreme grows), which reads as broken layout. The architectural path is canvas-based rendering with manual rasterization control.
